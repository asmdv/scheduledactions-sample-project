...
- Windows `computerName` must be <= 15 characters and valid by Windows naming rules.
- Set subnet `defaultOutboundAccess = false` explicitly if your policy audits/denies otherwise.
- For large requests, split by your service-side limit (max 100 VMs per request).
